...
- [ ] **3.5 The TUI.** Replicates terminal.wallhack.com one to one: mouse-clickable,
  ...
  frame: its own ASCII logo (not the vendor's mark, since the Wallhack logo is Wallhack's and this
  project is independent and unendorsed), the banner (`WALLHACK TERMINAL BY "@BRUX" - V<version>`),
