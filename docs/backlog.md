...
neither `wh socd list` nor the vendor UI can render: `wh socd list` refuses outright, naming the key
whose partner is not flagged, since a flag without a partner is exactly the inconsistency
`read_socd` is built to catch. Restoring a snapshot taken while the same pairs were live is fine;
restoring one across a `wh socd unpair`, or onto a board whose pairs have since changed, is not.
