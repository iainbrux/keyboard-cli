...
command that needs keyset membership, and written by `wh keyset create`, `wh keyset delete`, and by
`wh set ap` when a selection splits a keyset. `wh keyset set` changes only a keyset's value, never
its membership: it always passes no index to `keyset::plan`, so no membership record is ever sent.
