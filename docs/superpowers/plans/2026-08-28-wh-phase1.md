...
**Architecture:** Cargo workspace, four crates. `wh-proto` is a pure codec (all byte knowledge, no I/O); `wh-device` owns the `Transport` trait with real (`hidapi`), replay, and recording implementations plus a request/response `Session`; `wh-config` handles snapshots/backups/key-groups; `wh-cli` is the `clap` front end. Protocol is the Sparklink Playjoy platform, ported from the MIT-licensed `@sparklinkplayjoy/protocol-keyboard` TypeScript source vendored at `research/proto/package/src/` — cite exact upstream files when porting.
...
- Known-good frame (from decoded vendor templates): global-travel write starts `5C 0F 29 C9 01 00 00 …` — `0x35+0x5C+0x0F+0x29+0x00 = 0xC9`.
...
Expected: both succeed (hidapi compiles under mingw; if it fails, report — do not work around silently).
...
Note: `a-z` resolution order in the test is usage order because the range expands `0x04..=0x1D` and filters — resulting vec is ascending by usage. That matches the test's `[0x04, 0x07, 0x16, 0x1A]`.
...
Semantics: send one frame, then read until a valid reply whose `cmd` equals the request's `cmd` arrives (the interface also carries unsolicited input reports — skip anything that fails `parse` or has a different cmd). Give up after 8 reads or per-read timeout. `roundtrip_many` sends N frames and collects one reply per frame (used for batches).
...
Expected: success. (`cargo test -p wh-device` on Linux must still pass — hid.rs is cfg'd out.)
...
fn verify_rt_off<T: Transport>(s: &mut Session<T>, usages: &[u8]) -> Result<()> {
    ...
    if bad.is_empty() {
        ...
        bail!("readback mismatch on {} key(s) — backup retained, `wh restore --last` to roll back", bad.len())
    }
}
...
- [ ] **Step 7: First real write.** `bin/wh set rt --keys w --set 0.5 --dry-run` (user reviews frames against the captured ones — they should match modulo the mode-read), then without `--dry-run`. Verify in the web UI afterwards that it shows 0.5mm on W.
