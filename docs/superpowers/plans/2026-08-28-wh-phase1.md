...
- CMD sub-orders (`constants/param.ts`): `0x02` save-to-flash, `0x25` precision/min/max travel, `0x70` config select/query, `0x50` polling rate.
- Per-key record: `[hidUsage, layoutId, lo, hi]`, ≤14 records per report (`MaxPack = 0x0E`), payload prefixed by rw byte (`0x00` read / `0x01` write). Layout IDs: `0x04` AP (DB0), `0x08` mode (`value = touchMode<<4 | advancedMode`; touchMode `0x02` = RT), `0x14` RT-press, `0x15` RT-release.
...
- [ ] **Step 2: Integration test**: extend `crates/wh-cli/tests/` with a replay script for `set ap --keys w --set 1.2` covering: backup phase (full dump frames), write batch, SAVE, readback frames returning 1200. Assert exit 0 and "verified" in stdout. Also a mismatch variant (readback returns 1100) asserting exit ≠ 0 and "mismatch" in stderr. Reuse the Task 15 script-builder helpers.
...
- [ ] **Step 1: Capture.** Follow `capture/README.md` with the user driving the web UI: `initial-load`, `rt-on-w-0.5`, `rt-w-0.6`, `ap-w-1.2`, `rt-off-w`. Commit captures.
...
- [ ] **Step 3: Diff semantics.** Compare `rt-on-w-0.5` vs `rt-w-0.6` captures: exactly the RT_PRESS/RT_RELEASE values may differ (0x1F4 → 0x258). Confirm layout IDs and the SAVE order appear as this plan assumes (does the web app send order 0x02 after writes? does it write AP to layout 0x04 or a DBn variant?). Update `ops.rs` if reality differs, with test updates in the same commit.
