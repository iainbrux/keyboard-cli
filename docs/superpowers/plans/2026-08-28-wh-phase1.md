...
**Superseded, same as the Task 11 note above:** the `write_and_save` reuse here, the
`restore_all` call below, and its test expectation of a trailing SAVE frame are all historical.
The shipped code never sends order `0x02`; see `docs/protocol.md`.
...
**Superseded: the example scenario names below (`rt-w-0.6`, `ap-w-1.2`) are illustrative only.**
The session that actually ran used ten different scenario names; see the real `capture/README.md`
and `docs/protocol-inventory.md`. This block predates that session and is outside the top-of-plan
banner's stated scope (Tasks 11, 15, 16), so it carries its own note.
